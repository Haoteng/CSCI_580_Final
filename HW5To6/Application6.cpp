...
{
	GzRender* antiAlisingRenderer[AAKERNEL_SIZE];
	GzCamera    camera;
	...
	float AAFilter[AAKERNEL_SIZE][3] = { { -0.52, 0.38, 0.128 },
			{ 0.41, 0.56, 0.119 }, { 0.27, 0.08, 0.294 }, { -0.17, -0.29, 0.249 }, {
					0.58, -0.55, 0.104 }, { -0.31, -0.71, 0.106 } };
	...
	*/ 
	...
	//Antializing Renderer intializing

	for (int i=0;i<AAKERNEL_SIZE;i++){
		status |= GzDuplicateRender(render,&(antiAlisingRenderer[i])) ;
		antiAlisingRenderer[i]->shift[X]=AAFilter[i][X];
		antiAlisingRenderer[i]->shift[Y]=AAFilter[i][Y];
	}
	// I/O File open
	FILE *infile;
	...
	*/ 
	while( fscanf(infile, "%s", dummy) == 1) { 	/* read in tri word */
	    ...
	    */ 
	    ...
	    for (int i=0;i<AAKERNEL_SIZE;i++)
	    	GzPutTriangle(antiAlisingRenderer[i],3,nameListTriangle,valueListTriangle);
	} 

	GzResetDisplay(display);
	for (int i=0;i<AAKERNEL_SIZE;i++){
		GzAddDisplay(&render->display,antiAlisingRenderer[i]->display,AAFilter[i][2]);
		GzFreeDisplay(antiAlisingRenderer[i]->display);
		GzFreeRender(antiAlisingRenderer[i]);
	}
